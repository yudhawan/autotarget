...
function App() {
  ...
  const drop = e =>{
    ...
    if(target.length==0) settarget([{
      id:1,top:y,left:x,status:false,time:null,ip:'', timerRun:null
    }])
    if(target.length>0) {
  ...
  function playAction(){
    setTimeout(()=>{
      let newdata = target.filter((val)=> val.id!=target[i]?.id)
      let newrow = target.filter((val)=> val.id==target[i]?.id)[0]
      newrow.status=true
      newdata.push(newrow)
      settarget(newdata)
      ...
      // if(i==target.length-1) setplay(false)
    },1000)
    ...
    <div className="flex justify-between w-full h-full border border-slate-400 divide-x divide-gray-400">
            ...
            return(
              <div key={index+1} style={{left:item.left, top:item.top}} id={item.id} className={`w-24 p-1 h-48 rounded-md shadow-md border-gray-300 border flex flex-col justify-center items-center absolute `}>
                ...
                <div className='w-full bg-slate-100 rounded-md flex space-x-1 justify-center p-1'>
                  ...
                  <div className={`rounded-full bg-indigo-400 text-white w-5 h-5 p-1 cursor-pointer text-xs flex justify-center items-center`} onClick={()=>{
                    if(ip==item.id) {
                      setip(null)
                    }else{
                      setip(item.id)
      ...
      <div className="w-[15%] h-full flex flex-col justify-center items-center space-y-2 py-2">
        ...
        <div className='w-full h-full flex flex-col relative overflow-y-auto px-2 space-y-1'>
          {target.length>0&&<p className='underline text-sm text-slate-400 self-end cursor-pointer select-none' onClick={()=>settarget([])}>Clear</p>}
          {target?.map(item=>
          <div className='flex justify-between px-2 py-1 rounded-md border border-slate-400 w-full h-auto' >
            <p className='text-xs font-semibold text-gray-300 select-none cursor-pointer' onClick={()=>{
              if(ip==item.id) {
                setip(null)
              }else{
                setip(item.id)
        ...
        <div className='w-full h-[35rem] border-t border-white p-2 flex flex-col items-center space-y-1'>
          ...
          <div className='p-1 rounded-lg border border-violet-200 w-auto bg-white'><input className='outline-none w-full' value={namasesi} placeholder="Nama" type="text" onChange={(e)=> setnamasesi(e.target.value)}/></div>
          ...
          <div className='mt-1 flex flex-wrap gap-2 w-full'>
            {
              save?.map((val,index)=><div key={index} onClick={()=>{
                ...
                <p className='select-none'>{val.nama}</p>
